...
export default function TaskMapFull() {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-3 left-3 right-14 z-[1000] flex gap-1 sm:gap-1.5 overflow-x-auto">
        {filters.map((f) => (
          <button
            ...
            className="px-2.5 sm:px-3 py-1.5 rounded-full text-xs sm:text-xs font-semibold transition-all border shrink-0 min-h-[36px]"
            style={{
              background:
      ...
      {/* Stats overlay */}
      <div
        className="absolute bottom-4 left-3 z-[1000] rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-xs border border-white/10"
        style={{
