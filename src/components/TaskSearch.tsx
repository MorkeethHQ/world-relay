...
function SkeletonCard() {
  ...
  return (
    <div className="rounded-2xl p-4 flex flex-col gap-3 bg-white border border-gray-200 shadow-sm">
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-900/10 focus:border-gray-400 transition-all"
        />
        {query && (
          <button
            onClick={() => setQuery("")}
      ...
      <div className="flex items-center justify-between gap-2">
        <select
          ...
          className="bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-xs text-gray-500 focus:outline-none focus:ring-2 focus:ring-gray-900/10"
        >
      ...
      {showFilters && (
        <div className="flex items-center gap-2 animate-[fadeIn_0.2s_ease-out]">
          <span className="text-xs text-gray-400 shrink-0">Bounty:</span>
          <input
            type="number"
            value={minBounty}
            ...
            placeholder="Min $"
            min="0"
            className="w-20 px-2 py-1.5 bg-white border border-gray-200 rounded-lg text-xs text-gray-500 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-900/10"
          />
      ...
      {hasMore && !loading && (
        <button
          onClick={loadMore}
          className="w-full py-3 text-sm font-medium text-gray-500 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-all"
        >
          Load more tasks
        </button>
      )}
...
function TaskResultCard({ task }: { task: Task }) {
  ...
  return (
    <a
      href={`/task/${task.id}`}
      className="block rounded-2xl p-4 bg-white border border-gray-200 shadow-sm hover:shadow-md hover:border-gray-200 transition-all active:scale-[0.98]"
    >
      ...
      <div className="flex items-center gap-1.5 mb-3">
        ...
        <span className="text-xs text-gray-400 truncate">{task.location}</span>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          {/* Bounty badge */}
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-success-100 text-success-700 border border-success-200">
            ${task.bountyUsdc}
          </span>
          ...
          {task.agent && (
            <span
              className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-lg text-xs bg-gray-50 border border-gray-200"
              title={task.agent.name}
            >
              <span>{task.agent.icon}</span>
              <span className="text-gray-400 max-w-[60px] truncate">{task.agent.name}</span>
            </span>
          )}
        </div>
