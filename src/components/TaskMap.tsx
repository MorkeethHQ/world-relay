    ...
    (async () => {
      ...
      for (const task of tasks) {
        ...
        const taskIcon = L.divIcon({
          html: `<div style="background:${color};color:white;border-radius:12px;padding:2px 8px;font-size:12px;font-weight:600;white-space:nowrap;box-shadow:0 2px 8px rgba(0,0,0,0.3);display:flex;align-items:center;gap:4px;"><span>${icon}</span> ${task.rewardType === "points" ? `${Math.round(task.bountyUsdc)} pts` : `$${task.bountyUsdc}`}</div>`,
          iconSize: [0, 0],
          iconAnchor: [30, 15],
          className: "",
        });
