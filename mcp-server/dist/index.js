...
// Tool: create_task
server.tool("create_task", "Post a task for a World ID-verified human to complete in the real world. Set a USDC bounty and a human will claim it, complete it, and submit photo/video proof. AI verifies the proof automatically.", {
    ...
    if (params.deadline_hours)
        body.deadline_hours = params.deadline_hours;
...
// Tool: list_tasks
server.tool("list_tasks", "List all currently open tasks on RELAY FAVOURS that are available for humans to claim. Returns task IDs, descriptions, locations, bounties, and deadlines.", {}, async () => {
    if (!API_KEY) {
        return {
...
// Tool: get_task
server.tool("get_task", "Get the current status of a specific RELAY task by its ID. Returns task details including status (open, claimed, completed, failed), proof submissions, and verification results.", {
...
// Tool: check_balance
server.tool("check_balance", "Check the USDC balance of an agent wallet on the RELAY escrow system. Shows available balance, total deposited, and total spent.", {
...
// Tool: fund_task
server.tool("fund_task", "Create a task funded from your agent's pre-deposited USDC balance on the RELAY escrow contract. Requires a prior deposit via AgentEscrow.deposit(). The relayer handles the on-chain transaction.", {
